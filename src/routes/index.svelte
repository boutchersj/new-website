<nav class="flex w-full justify-evenly text-yellow-500">
    <a href="/">About Me</a>
    <a href="/education">Education</a>
    <a href="/portfolio">Portfolio</a>
</nav>
<br />
<hr />

<header class="flex flex-col items-center my-5">
    <h1 class="text-4xl mb-2">Steven Boutcher</h1>
    <h2 class="text-xl">QA | Web Dev | Educator</h2>
</header>

<h3 class="text-xl text-center mb-5">About Me</h3>
<section class="mx-5 h-full border border-solid border-purple-500 p-5 rounded-xl">
    <p>
        For those interested in my full tech career journey, go 
        <a class="text-yellow-500" href="https://blog.stevenboutcher.com/my-tech-career-origin-story-part-i">
            here
        </a>
    </p>
    <br />
    <p>
        For the rest of you, hi, I'm Steven! Thanks for stopping by!
    </p>
    <br />
    <p>
        During my time in tech, I've worn a few hats, including customer support, manual software testing, frontend development, 
        interviewing, onboarding, mentoring, and documentation. I'm always down for learning new things at work, and I've become 
        a fan of adding to my company's internal wiki documentation to help with onboarding new QA Engineers and make it easier 
        for my coworkers to acquire new skills.
    </p>
    <br />
    <p>
        Outside of work, my wife, Sam & I have 2 adorable kitty cats named Minnie & Sadie, and we love to travel, cook, go to the movies, and try new things together. 
        We saw Seattle and Las Vegas for the first time on our honeymoon, and 5 months later we took our first overseas trip to Spain 
        to visit Sevilla and Granada!
    </p>
    <br />
    <p>
        Fun fact: Sam and I started together on the customer support team at Fetch Rewards, but we didn't meet there!
    </p>
    <br />
    <p>

        If you'd like to chat over coffee over Zoom, drop me an 
        <a class="text-yellow-500" href="mailto:boutchersj@gmail.com">
            email
        </a>.
    </p>
</section>